	...
	for (int y = 0; y < this->depth.rows; y += 5) {
		...
		for (int x = 0; x < this->depth.cols; x += 5) {
			...
			if (z != 0 && z < 1024 + 512) {
				...
				float noise_x = ofNoise(x * 0.005, ofGetFrameNum() * 0.05);
				float noise_y = ofNoise(y * 0.005, ofGetFrameNum() * 0.05);
				float noise_z = ofNoise(z * 0.005, ofGetFrameNum() * 0.05);
				float gap = 100;

				ofPoint noise_point = ofPoint(ofMap(noise_x, 0, 1, -gap, gap), ofMap(noise_y, 0, 1, -gap, gap), ofMap(noise_z, 0, 1, 0, -gap * 2));
				
				if (noise_point.length() < gap) {

					ofSetColor(ofMap(noise_x, 0, 1, 0, 255), ofMap(noise_y, 0, 1, 0, 255), ofMap(noise_z, 0, 1, 0, 255));
					ofDrawLine(ofPoint(), noise_point);
					ofDrawBox(noise_point, 8);
				}
				else {
